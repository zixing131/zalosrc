package com.google.android.gms.auth.api.accounttransfer;

import android.os.Parcel;
import android.os.Parcelable;
import com.google.android.gms.common.internal.safeparcel.SafeParcelReader;
import java.util.ArrayList;
import java.util.HashSet;

/* renamed from: com.google.android.gms.auth.api.accounttransfer.a */
/* loaded from: classes2.dex */
public final class C4020a implements Parcelable.Creator {
    @Override // android.os.Parcelable.Creator
    public final /* synthetic */ Object createFromParcel(Parcel parcel) {
        int m19765N = SafeParcelReader.m19765N(parcel);
        HashSet hashSet = new HashSet();
        ArrayList arrayList = null;
        zzo zzoVar = null;
        int i11 = 0;
        int i12 = 0;
        while (parcel.dataPosition() < m19765N) {
            int m19755D = SafeParcelReader.m19755D(parcel);
            int m19789v = SafeParcelReader.m19789v(m19755D);
            if (m19789v != 1) {
                if (m19789v != 2) {
                    if (m19789v != 3) {
                        if (m19789v != 4) {
                            SafeParcelReader.m19764M(parcel, m19755D);
                        } else {
                            zzoVar = (zzo) SafeParcelReader.m19782o(parcel, m19755D, zzo.CREATOR);
                            hashSet.add(4);
                        }
                    } else {
                        i12 = SafeParcelReader.m19757F(parcel, m19755D);
                        hashSet.add(3);
                    }
                } else {
                    arrayList = SafeParcelReader.m19787t(parcel, m19755D, zzr.CREATOR);
                    hashSet.add(2);
                }
            } else {
                i11 = SafeParcelReader.m19757F(parcel, m19755D);
                hashSet.add(1);
            }
        }
        if (parcel.dataPosition() == m19765N) {
            return new zzl(hashSet, i11, arrayList, i12, zzoVar);
        }
        StringBuilder sb2 = new StringBuilder(37);
        sb2.append("Overread allowed size end=");
        sb2.append(m19765N);
        throw new SafeParcelReader.ParseException(sb2.toString(), parcel);
    }

    @Override // android.os.Parcelable.Creator
    public final /* synthetic */ Object[] newArray(int i11) {
        return new zzl[i11];
    }
}
